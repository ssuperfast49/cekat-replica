...
- Changed base urls from hard coded "https://cssuper.com" to based on the ongoing baseurl redirect
...
- Added RPC `get_threads_with_details` to optimize threads fetching and eliminate N+1 queries.
- Added explicit explicit type casting to enum parameters in `get_threads_with_details` to prevent Postgres operator mismatch errors.

### Fixed

- **Fixed Duplicate Fetching Bug**: Resolved a severe Thundering Herd issue where `useConversations` cascading updates caused 15+ simultaneous duplicate queries (`get_threads_with_details` and `get_tab_counts_v3`) on initial mount by introducing batched network debouncing and strict execution locks.

### Removed

- Removed metrics telemetry. Removed `circuit_breaker_metrics` table, `CircuitBreakerStatus` component, and all related telemetry logging logic to reduce database I/O contention. entirely. Stripped out all background metric recording interceptors from the `protectedSupabase` client wrapper. Removing this extremely high-volume write mechanism dramatically reduces database CPU load, eliminates the `app/lib/metrics.ts` overhead, and eradicates the UI lag that occurred when the Circuit Breaker dashboard previously performed full-table aggregations.

### Bug Fixes & Stability

- **RPC Enum Casting Safe Guards**: Systematically added explicit text casting (`::text`) to database ENUM mappings (`channel_type`, `thread_status`, `message_role`, `message_direction`) within the new threads RPC. This completely prevents `operator does not exist` and `invalid input value for enum` PostgreSQL parser crashes when the frontend natively queries with text filters.
...
- **Admin Dashboard Scaling**: Expanded the allowable configuration ranges in the Circuit Breaker status panel (supporting up to 10,000 requests/min) to accommodate large agent teams.
- **AI Wallet Reset**: Manually restored AI Wallet balance to $160.00 (100% battery) on the Main production branch.

## [0.3.4] - AI Wallet & UI Optimizations

### Fixed

- **AI Wallet Auto-Deductions**: Fixed a major bug where the PostgreSQL database trigger (`apply_wallet_cost_from_token_log`) failed to recognize versioned AI model names (e.g. `gpt-5.4-mini-2026-03-17`) generated by the OpenAI API. The trigger now utilizes a smart prefix-matching algorithm (`LIKE`) to accurately anchor versioned model strings to their master pricing tiers.
- **Retroactive Cost Recovery**: Applied a one-time retroactive deduction to correct the wallet anomaly, restoring accurate token accounting in Production.

### Changed

- **Infinite Scroll for Active Inboxes**: Permanently removed the rigid 20-thread pagination limit strictly for the `Assigned` and `Unassigned` tabs. These high-priority queues now execute a boundless query (`LIMIT 10000`), downloading and rendering the entire stack of open threads instantaneously. Pagination interface logic automatically suspends itself for these scopes.

## [0.3.3] - Database Analytics & Optimization Updates

### Performance

- **Server-Side Token Analytics**: Replaced heavy client-side aggregation of `token_usage_logs` with a blazing fast PostgreSQL RPC (`get_token_usage_stats`). The `Analytics.tsx` frontend now retrieves tiny pre-aggregated JSON arrays, preventing browser crashes and bandwidth spikes on large 30-day datasets.
- **Record Level Security (RLS) Tuning**: Optimized heavily-queried `messages` and `threads` RLS policies. Migrated from the volatile `auth.uid()` function to `(select auth.uid())`, enabling Postgres to cache queries and bypass per-row ID re-evaluation, drastically speeding up inbox load times.
- **Index Cleanup**: Dropped 3 redundant/duplicate indexes (`idx_channels_super_agent_id`, `idx_super_agent_members_org_agent`, `idx_super_agent_members_super`) to free up storage space and remove overhead during database `INSERT` operations.
...
- **Low Battery Alert Modal**: When battery drops below 20%, a popup modal appears for all agents every 10 minutes in Indonesian language:
  - Master Agents: *"Silakan segera lakukan top up saldo AI wallet..."*
  - Other Agents: *"Silakan hubungi Master Agent untuk melakukan top up..."*
...
- **Bug Fixes & UX Polish**:
  ...
  - **Welcome Message Timing**: Moved the welcome message insertion to fire *before* the AI webhook call. This guarantees the welcome bubble renders instantly before the AI response starts streaming.
